...
// 回傳 { list, result }：list 是新陣列（沒變就是原陣列），result 是給模型的回覆
export function addMemory(list, text) {
  const t = String(text ?? "").replace(/\s+/g, " ").trim();
  if (!t) return { list, result: "沒有內容可以記。" };
  if (t.length > MAX_MEMORY_CHARS) return { list, result: `太長了（${t.length} 字），請濃縮成 ${MAX_MEMORY_CHARS} 字內的一句話。` };
  if (list.includes(t)) return { list, result: "已經記得這件事了。" };
  if (list.length >= MAX_MEMORIES) return { list, result: `記憶已滿（${MAX_MEMORIES} 條），請先用 forget 刪掉過時的。` };
  return { list: [...list, t], result: `已記住：${t}` };
}

// 完全相同優先；否則找唯一一條包含這段文字的
export function forgetMemory(list, text) {
